...
import React, { useStateEffect, useCallback, useRef } from "react";
import {
  Shield,
 Loader2,
  ArrowRight,
...
function InlineCopy({ text, title }: { text: string; title?: }) {
  const [copied, setCopied] = useState(false);
  return (
    <button
      onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() =>Copied(false), 2000); }}
      className={`p-1.5 rounded-lg transition-all flex-shrink-0 ${copied ? "bg-green-500 text-white" : "text-slate-300 hover:text-indigo-500 hover:bg-indigo50"}`}
      title={title ?? (copied ? "已复制" : "复制链接")}
    >
      {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h" />}
    </button>
  );
}
...
export default function AdminPage() {
  ...
  const [passwordLoading, setPassword] = useState(false);
  ...
  const [editIcon, setEdit] = useState<File | null>(null);
  const [editIconPreview, setEditIconPreview] = useState<string | null>(null);
  const [savingEdit, setSavingEdit] = useState(false);
  const iconInputRef = use<HTMLInputElement>(null);
  ...
  useEffect(() => {
    fetch("/api/admin/auth")
      .then((r) => r.json())
      .then((data) => {
        if (!data.enabled          setAuthState("disabled");
 ...
 const handleSyncCollection = async (col: Collection) => {
    ...
    try {
      ...
      if (!fwd.widgets || !Array.is(fwd.widgets)) throw new Error("Invalid .fwd");
      ...
      for (const widget of fwd.widgets) {
        ...
        widgetMetas.push({ id: widget.id, title.title, description: widget.description version: widget.version, author.author requiredVersion: widget.requiredVersion, source_url: widget.url });
      }
 const formData = new Form();
      ...
      formData.append("source_url", col.source);
 ...
 try {
      ...
      if (res.ok) {
        setCollections((prev) => prev.filterc) => c.id !== col));
      }
    } finally {
      setDeletingId(null);
    }
...
IconPreview(null);
  ...
  if (authState === "loading") {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <Loader2 className="w-8 h8 animate-spin text-slate-400" />
      </div>
    );
  }
 ...
 if (authState === "disabled") {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border-slate-200 p8 text-center space-y-4">
  ...
  if (authState === "need-password") {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border-slate-200 p8 space-y6">
          <div className="text-center space-y-2">
            <div className="mx-auto w-12 h-12 bg-orange50 rounded-full flex items-center justify-center">
              <Shield className="w-6 h-6 text-orange600" />
            </div>
 ...
 placeholder="请输入管理员密码"
              className="w-full px-4 py-3 border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2-orange-500 focus:border-transparent"
              autoFocus
            />
            {passwordError && (
              <p className="text-sm text-red-500 flex items-center gap1">
                <AlertCircle className="w-3.5 h-.5" />
                {passwordError}
              </p>
            )}
 <button
              onClick={handlePasswordSubmit}
              disabled={passwordLoading || !Input.trim()}
              className="w-full py-3 bg-orange-600 text-white text-sm font-medium rounded-xl hover:bg-orange-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify gap-2"
            >
              {passwordLoading ? (
 ...
 return (
    <div className="min-h-screen bg-slate-50 text-s-800 p4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
       ...
       div>
        ...
        {!collectionsLoading && collections.length === 0 && (
          <div className="text-center py-12 text-slate-400-sm">
            暂无合集
          </div>
        )}
        ...
        {collections.map((col) => (
          <div
            key={col.id}
            className="bg-white rounded-2xl shadow-sm border-slate-200 overflow-hidden"
          >
            <div className="px-6 py4 border-b border-slate-100 bg-slate-50/50">
              {editingColId === col.id ? (
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="w-9 h9 rounded-lg border-2-dashed-slate-300 hover:border-indigo-400 flex items-center justify flex-shrink-0 overflow-hidden transition-colors"
                      title="更换图标"
                    >
                      {editIconPreview ? (
                        <img src={editIconPreview} alt="" className="w-full h object-cover" />
                      ) : col.icon_url ? (
                        <img src={col.icon_url} alt="" className="w-full h-full object-cover opacity-60" />
                      ) : (
                        <ImagePlus className="w-4 h text-slate-400" />
                      )}
                    </button>
                    <div className="flex-1 space-y-2">
                      <input
                        ...
                        className="w-full text-sm font-semibold-slate-800 border-slate-200 rounded-lg px-3 py1.5 focus:outline-none focus:ring-2ring-indigo-500 focus:border-transparent"
                        placeholder="合集标题"
                      />
 <input
                        type="text"
                        ...
                        className="w-full text-xs-slate-500 border-slate-200 rounded-lg px-3 py1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        placeholder="合集描述（可选）"
                      />
                    </div>
                    ...
                    <
                      ...
                      className="px-3 py-1.5 text-xs-white bg-indigo-600 hover:bg-indigo700 rounded-lg transition-colors disabled:opacity-50"
                  ...
                  <div className="flex items-center gap-3">
                    {col.icon_url ? (
                      <img
                        src={col.icon_url}
                        alt=""
 className="w-9 h- rounded-lg object-cover"
                      />
                    ) : (
                      <div className="w-9 h9 rounded-lg bg-indigo-50 flex items-center justify">
                        <FileJson className="w-4.5 h-4.5 text-indigo600" />
                      </div>
                    )}
 <div>
 ...
 <div className="flex items-center gap-1 flex-shrink-0">
                    <span className="text-xs text-slate-500 bg-slate-100 px2 py-1 rounded-md mr-1">
 ...
 {col.source_url && (
                      <button
                        ...
                        className="p-1.5 text-slate-300 hover:text-emerald-500:bgerald-50 rounded-lg transition-colors disabled:opacity-50"
                        title="从源地址同步"
                      >
           ...
           >
            ...
            <div className="px-6 py-2 bg-indigo50/40 border-b-slate-100 flex items-center gap-2">
              <Key className="w-3.5 h-.5 text-indigo-400 flex-shrink0" />
              <span className="text-xs text-indigo-600 font-medium flex-shrink0">订阅链接</span>
              <input type="text" readOnly value={`${typeof window !== "undefined" ? window.location.origin : ""}/api/collections/${col.slug}/fwd`} className="bg-transparent text-xs font-mono-indigo-500 flex-1 min-w-0 focus:outline-none truncate" />
 ...
 < className="divide-y divide-slate-100">
              {col.modules.map((mod) => (
                ...
                >
                  <div className="p-2 rounded-lg bg-amber50 text-amber600 flex-shrink-0">
                    <FileCode className="w-4 h" />
                  </div>
                  <div className="min-w-0 flex1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-sm-slate-800 truncate">
                        {mod.title || mod.filename}
                      </span>
                      {mod.version && (
                        <span className="text-xs bg-slate-100 textlate-600 px-1.5 py-0.5 rounded">
                          {mod.version}
                        </span>
                      )}
 {mod.is_encrypted ? (
                        <span className="text-xs bg-purple-50 text600 px1.5 py-0.5 rounded border-purple-100">
                          加密
                        </span>
                      ) : null}
                    </div>
                  ...
                  < className="flex items-center gap-1 flex-shrink-0">
                    <InlineCopy text={`${typeof window !== "undefined" ?.location.origin : ""}/api/modules/${mod.id}/raw`} title="复制模块链接" />
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate300 hover:text-indigo-500 hover:bgigo-50 rounded-lg transition-colors disabled:opacity-50"
                      title="更新文件"
                    >
                      {replacingModuleId === mod.id ? <Loader2 className="w-4 h- animate-spin" /> : <UploadCloud className="w-4 h4" />}
                    </button>
                    {mod.source_url && (
                      <button
                        disabled={syncingModuleId === mod.id}
                        onClick={() => handleSyncModule(mod)}
                        className="p-1.5 text-slate-300 hover:text-emerald-500 hover:bg-emerald50 rounded-lg transition-colors disabled:opacity-50"
                        title="从源地址同步"
                      >
 ...
 <button
                      ...
                      title="删除模块"
                    >
                      {deletingModuleId === mod.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h" />}
                    </button>
                 div               div>
              ))}
              {col.modules.length === 0 && (
                <div className="px-6 py-4 text-center-sm text-slate-400">
                  暂无模块
                </div>
              )}
           >
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/30 flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm-slate-500 hover:text-indigo600 transition-colors disabled:opacity-50 flex-shrink0"
              >
                {uploadingColId === col.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                {uploadingColId === col.id ? "上传中..." : "添加模块"}
              </button>
 <div className="flex items-center gap-1.5 flex min-w-0">
                <Globe className="w-3.5 h-3.5 text-slate-400 flex-shrink0" />
                <input
                  type="text"
                  ...
                  className="flex-1 min-w0 text-xs-slate-600 placeholder:text-slate-400 bg-transparent border-slate-200 rounded-md px-2 py-1.5 focus:outline-none focus:border-indigo-400"
                />
