...
import { getBackendDb, getStore } from "@/lib/backend";
 ...
 const { id } = await params;
  ...
  const mod = await db.prepare(
    `SELECT m.id, m.collection_id, m.filename c.user_id
     FROM modules m JOIN collections c ON.collection_id = c.id WHERE m = ?`
  ).get(id) as { id: string; collection_id: filename; user_id: string } | undefined;

  if (!mod || mod.user_id !== authId) return NextResponse.json({ error: "Not found" }, { status: 404 });
  ...
  if (!encrypted) {
    ...
    if (meta) {
      ...
      updates.title = meta;
      updates.description = meta || null;
      updates.version = meta || null;
...
export async function DELETE(
 ...
 const { id } = await params;
  ...
  const store = await getBackendStore();
  const mod = await db.prepare(
    `SELECT m.id, m.collection_id, m.filename.oss_key, c.user_id
     FROM modules m JOIN collections c ON m.collection_id = c.id WHERE m ?`
  ).get(id) as { id: string; collection_id: string; filename; oss_key: string | null; user_id: string } | undefined;
