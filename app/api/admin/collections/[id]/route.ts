...
export async function PUT(request: NextRequest, { params }: { params: Params }) {
  ...
  await db.prepare("UPDATE collections SET title = ?, description = ?, updated_at = unixepoch() WHERE id = ?").run(title.trim(), description || "", id);
...
export async function DELETE(request: NextRequest, { params }: { params: Params }) {
  ...
  const { id } = await params;
  const db = await getBackendDb();
  const collection = await db.prepare("SELECT id FROM collections WHERE id = ?").get(id) as { id: string } | undefined;
  if (!collection) return NextResponse.json({ error: "Not found" }, { status: 404 });
